...
void current_pos_cb(const gazebo_msgs::ModelStates::ConstPtr& msg){
    ...
    tf::Quaternion q(
        current_pos.pose[2].orientation.x,
        current_pos.pose[2].orientation.x,
        current_pos.pose[2].orientation.z,
        current_pos.pose[2].orientation.w
    );
    tf::Matrix3x3 m(q);
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher local_yaw_pub = nh.advertise<geometry_msgs::TwistStamped>
            (followerNS + "/mavros/setpoint_velocity/cmd_vel", 10);
    ...
    while(ros::ok()){
        if( current_state.mode != "OFFBOARD" &&
            ...
            if(current_state.mode == "OFFBOARD"){
                ...
                double f_curr_x = current_pos.pose[2].position.x;
                double f_curr_y = current_pos.pose[2].position.y;

                double t_curr_x = current_pos.pose[1].position.x;
                double t_curr_y = current_pos.pose[1].position.y;
                ...
                ROS_INFO("Current yaw is: %f", curr_yaw);

                double actual_angle_to_yaw = angle_to_target - curr_yaw;
                ROS_INFO("Angle required to yaw: %f", actual_angle_to_yaw);
                
                if (actual_angle_to_yaw > degree_of_error){
                    yaw.twist.linear.x = 0;
                    yaw.twist.angular.z = 3.14159/6;
                }
